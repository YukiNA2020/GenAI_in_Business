...
Widget _profileStatsRow(ProfileStatsDisplay display) {
  return Row(
    children: [
      _StatCell(label: 'Exhibits', value: display.exhibitsLabel),
      _StatCell(label: 'Rooms', value: display.roomsLabel),
      _StatCell(label: 'Public', value: display.publicLabel),
      _LastAddedCell(display: display.lastAdded),
    ],
  );
}
      ...
      loading: () {
        if (list.items.isNotEmpty) {
          final display = resolveProfileStatsDisplay(
            museumCatalogItems: list.items,
            stats: demoUserStatsFallback,
          );
          return _StatsCard(child: _profileStatsRow(display));
        }
        return const Center(
          child: Padding(
