...
/** 从 collections.tags 列聚合标签（SQLite） */
async function findDistinctTags() {
  const db = await getDb();
  const result = db.exec('SELECT tags FROM collections WHERE tags IS NOT NULL');
  const set = new Set();
  if (result.length && result[0].values.length) {
...
module.exports = { insert, findById, findAll, update, remove, findDistinctTags };
